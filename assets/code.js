...
// Edit a task
document
  .querySelectorAll("button[class='edit_task_button'")
  .forEach((editBtn) => editBtn.addEventListener("click", editTask));
function editTask() {
...
document
  ...
  .forEach((box) => box.addEventListener("click", completeTask));
...
document
  ...
  .forEach((delBtn) => delBtn.addEventListener("click", deleteTask));
function deleteTask() {
...
// Edit title list
document
  .querySelector(".edit_title_button")
  .addEventListener("click", editTitleList);
function editTitleList() {
  this.previousElementSibling.setAttribute(
    "class",
    "title_editable input_title"
  );
  this.previousElementSibling.setAttribute("contenteditable", "");
  this.innerHTML = '<i class="fa fa-check" aria-hidden="true"></i>';
  this.style.backgroundColor = "mediumseagreen";
